// base-devnet-setup generates the offline Base L1/L2 devnet configuration.
package main
...
func run() error {
	if len(os.Args) > 1 {
		if len(os.Args) == 3 && os.Args[1] == "extract-artifacts" {
			_, err := artifacts.ExtractEmbedded(os.Args[2])
			return err
		}
		return fmt.Errorf("usage: base-devnet-setup [extract-artifacts DIRECTORY]")
	}

	if env("L2_DEPLOYMENT_TARGET", "genesis") != "genesis" {
		return fmt.Errorf("base-devnet-setup supports offline genesis only")
	}
	...
	values := map[string]string{
		"CHAIN_ID": l1ID, "L1_CHAIN_ID": l1ID, "L2_CHAIN_ID_HEX": fmt.Sprintf("0x%064x", l2ID),
	...
	intentData, err := template("l2-intent.toml.template", values)
	if err != nil {
		return err
	}
	var intent state.Intent
	if err := toml.Unmarshal(intentData, &intent); err != nil {
		return err
	}
	if path := os.Getenv("BASE_DEVNET_ARTIFACTS"); path != "" {
		locator, err := artifacts.NewLocatorFromURL("file://" + path)
		if err != nil {
			return err
		}
		intent.L1ContractsLocator = locator
		intent.L2ContractsLocator = locator
	}
	...
	if err := deployer.ApplyPipeline(context.Background(), deployer.ApplyPipelineOpts{
		DeploymentTarget: deployer.DeploymentTargetGenesis, Intent: &intent, State: st, Logger: logger,
		StateWriter: pipeline.NoopStateWriter(), CacheDir: cache,
	}); err != nil {
		return err
	}
	...
	addresses, err := inspect.L1(st, intent.Chains[0].ID)
	if err != nil {
		return err
	}
	for _, output := range []struct {
		...
		{filepath.Join(l2Dir, "l1-addresses.json"), addresses},
	} {
